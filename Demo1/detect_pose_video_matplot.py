...
"""
This file runs the camera for a certain number of iterations (it's set to 100 at the time of this writing),
and it saves the landmarks at each frame. It then proceeds to iterate over them and draw them in PLT in real
time (with a configurable frame rate). Thus it works with "video", but it is delayed and does not run simultaneously.
The reason for this is that it was inconvenient to do it simultaneously given the structure callback function for
MediaPipe. We can configure that if we decide it's useful in the future. For the time being this is a proof of concept.
"""
